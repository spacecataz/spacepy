...
def _trace3d_common(func, fieldx, fieldy, fieldz, xstart, ystart, zstart,
                    gridx, gridy, gridz, maxstep=200000, ds=0.01):
    # Same as for _trace2d_common, but for 3D files.
    gridx = numpy.require(gridx, ctypes.c_double, 'C')
    gridy = numpy.require(gridy, ctypes.c_double, 'C')
    gridz = numpy.require(gridz, ctypes.c_double, 'C')
    fieldx = numpy.require(fieldx, ctypes.c_double, 'C')
    fieldy = numpy.require(fieldy, ctypes.c_double, 'C')
    fieldz = numpy.require(fieldz, ctypes.c_double, 'C')
    outx = numpy.empty(shape=(maxstep,), dtype=ctypes.c_double)
    outy = numpy.empty(shape=(maxstep,), dtype=ctypes.c_double)
    outz = numpy.empty(shape=(maxstep,), dtype=ctypes.c_double)
    count = func(gridx.shape[0], gridy.shape[0], gridz.shape[0], maxstep, ds,
                 xstart, ystart, zstart, gridx, gridy, gridz,
                 fieldx, fieldy, fieldz, outx, outy, outz)
    return (outx[0:count], outy[0:count], outz[0:count])
...
def trace3d_rk4(fieldx, fieldy, fieldz, xstart, ystart, zstart,
                gridx, gridy, gridz, maxstep=20000, ds=0.01):
    """
    Given a 3D vector field, trace a streamline from a given point
    to the edge of the vector field.  The field is integrated using
    Runge Kutta 4.  Only valid for regular grid with coordinates gridx, 
    gridy, and gridz. If grid* values are not given, assume that 
    *start arguments are normalized coordinates (e.g., position in terms of 
    array indices.)
    """
    return _trace3d_common(lib.cRk4_3d, fieldx, fieldy, fieldz,
                           xstart, ystart, zstart,
                           gridx, gridy, gridz, maxstep, ds)
...
def test_asymtote3d():
    '''
    Test streamline tracing by plotting vectors and associated streamlines
    through a simple velocity field where Vx=x, Vy=y, Vz=-z.
    '''
    import numpy as np
    import matplotlib.pyplot as plt

    # Start by creating a velocity vector field.
    xmax, ymax, zmax = 50.0, 50.0, 20.0
    x = np.arange(0.01, xmax + 0.25, 0.25)
    y = np.arange(0.01, ymax + 0.25, 0.25)
    z = np.arange(0.01, zmax + 0.25, 0.25)
    ...
    x_anly = np.arange(.1, xmax, 0.05)
    y_anly = np.arange(.1, xmax, 0.05)
    z_anly = 1.0 / (const * x_anly * y_anly)
